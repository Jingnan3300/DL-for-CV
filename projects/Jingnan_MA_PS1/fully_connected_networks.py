...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dx = dout.mm(w.t()).reshape(x.shape[0], *x.shape[1:])
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        b2 = self.params['b2']
        N, D = X.shape
        hidden_layers = torch.relu(X.mm(W1) + b1)
        scores = hidden_layers.mm(W2) + b2
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dscores /= X.shape[0]
        dW2 = hidden_layers.t().mm(dscores) + 2 * self.reg * W2
        db2 = dscores.sum(dim=0)
        
        dhidden = dscores.mm(W2.t())
        dhidden[hidden_layers <= 0] = 0
        dW1 = X.t().mm(dhidden) + 2 * self.reg * W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers+1):
            self.params[f'W{i}'] = torch.randn(dims[i-1], dims[i], device=device, dtype=dtype) * weight_scale
            self.params[f'b{i}'] = torch.zeros(dims[i], device=device, dtype=dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            W_num_layers = self.params[f'W{self.num_layers}']
            b_num_layers = self.params[f'b{self.num_layers}']
            scores, cache[f'{i}'] = Linear_ReLU.forward(scores, W_i, b_i)
        scores, cache[f'{self.num_layers}'] = Linear.forward(scores, W_num_layers, b_num_layers)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        dscores, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, cache[f'{self.num_layers}'])
        for i in range(self.num_layers-1, 0, -1):
            dscores, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dscores, cache[f'{i}'])
