...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    test_vals = x_test.view(num_test, -1)
    train_vals = x_train.view(num_train, -1)
    for i in range(num_train):
      dists[i, :] = torch.sum((train_vals[i] - test_vals)**2, dim = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_sq = torch.sum(train_vals ** 2, axis=1).view(-1, 1)
    test_sq = torch.sum(test_vals ** 2, axis=1).view(1, -1)

    dotp = torch.mm(train_vals, test_vals.t())

    dists = train_sq + test_sq - 2 * dotp
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    label = torch.zeros(num_test, k, dtype=torch.int64)
    for j in range(num_test):
      _, index = torch.topk(dists[:, j], k, largest=False)
      label[j, :] = y_train[index]
      label_counts = torch.bincount(label[j, :], minlength = num_test)
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        ## Set up testing sets & training sets
        x_test_set = x_train_folds[i]
        y_test_set = y_train_folds[i]

        x_train_set = torch.cat(x_train_folds[:i] + x_train_folds[i+1:])
        y_train_set = torch.cat(y_train_folds[:i] + y_train_folds[i+1:])

        ## Initialize new KnnClassifier
        new_classifier = KnnClassifier(x_train_set, y_train_set)

        dist = compute_distances_no_loops(new_classifier.x_train, x_test_set)
        pred_vals = predict_labels(dist, new_classifier.y_train, k)

        accuracy = (pred_vals == y_test_set).float().mean().item()
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracy in k_to_accuracies.items():
      ...
      if avg > op_accuray or (avg == op_accuray and (best_k != None or k < best_k)):
        best_k = k
        op_accuray = avg
    ##########################################################################
    #                           END OF YOUR CODE                             #
