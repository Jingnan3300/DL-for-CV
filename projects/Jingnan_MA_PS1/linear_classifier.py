...
def test_one_param_set(
    ...
    train_acc = (train_pred == data_dict['y_train']).float().mean().item()

    val_pred = cls.predict(data_dict['X_val'])
    val_acc = (val_pred == data_dict['y_val']).float().mean().item()
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
      score = X[i].unsqueeze(0).mm(W)                     
      score -= torch.max(score)
      y_log = score[0, y[i]]
      y_exp = torch.exp(y_log)
      exp_sum = torch.sum(torch.exp(score), dim=1, keepdim=True)                  
      loss += -torch.log(y_exp / exp_sum)

      for j in range(num_classes):
        prob = torch.exp(score[0, j]) / exp_sum.squeeze()
        if j == y[i]:
          dW[:, j] += (prob - 1) * X[i].t()
        else:
          dW[:, j] += prob * X[i].t()
...
def softmax_loss_vectorized(
    ...
    score -= torch.max(score)
    p = torch.exp(score)
    p /= torch.sum(p, axis=1, keepdim=True)

    loss = -torch.sum(torch.log(p[range(num_train), y])) / num_train
    loss += reg * torch.sum(W * W)

    p[range(num_train), y] -= 1
    dW = X.t().mm(p) / num_train 
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 1e-2, 1e-7, 5e-7, 5]
    regularization_strengths = [0.01, 0.1, 0.5, 1, 10]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
